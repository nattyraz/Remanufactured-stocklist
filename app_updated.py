...
def get_table_download_link(df, title="Download Excel file", filename="data.xlsx"):
    val = to_excel(df)
...
# Update display_data_page function to add interactive checkboxes and download functionality
def display_data_page():
    # Your existing setup for the display page
    # ... (col1, col2, combined_data, last_update_date, etc.)
    
    # Initialize session state for checkboxes
    if 'selected_rows' not in st.session_state:
        st.session_state.selected_rows = [False for _ in range(len(filtered_data))]

    # Create an interactive table with checkboxes
    for i, row in filtered_data.iterrows():
        checkbox_state = st.checkbox(f"Select {row['No.']}", value=st.session_state.selected_rows[i], key=i)
        st.session_state.selected_rows[i] = checkbox_state
        st.write(row[columns_to_display])

    # Add a button to download selected rows
    if st.button('Download Selected Rows'):
        selected_rows = filtered_data[[st.session_state.selected_rows[i] for i in range(len(filtered_data))]]
        st.markdown(get_table_download_link(selected_rows), unsafe_allow_html=True)
