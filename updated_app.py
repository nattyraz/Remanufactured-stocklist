...
def advanced_filter_data_by_search_query(df, query):
    ...
    filtered_data = df[df['Description'].str.contains(pattern) | df['No.'].astype(str).str.contains(pattern)]
    
    # Always include graphics "01" and "02"
    graphics_01_02 = df[df['Description'].str.contains("01|02")]
    combined_data = pd.concat([filtered_data, graphics_01_02]).drop_duplicates()
...
def display_data_page():
    ...
    if last_update_date:
        st.write(f"Last updated: {last_update_date.strftime('%Y-%m-%d %H:%M:%S')}")
...
# Formatting the price columns
for column in currency_columns:
    if "EUR" in column:
        df[column] = df[column].str.replace(",", "").astype(str) + " €"
    elif "GBP" in column:
        df[column] = df[column].str.replace(",", "").astype(str) + " £"
    elif "DKK" in column:
        df[column] = df[column].str.replace(",", "").astype(str) + " Kr"
