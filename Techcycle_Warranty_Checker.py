...
# Page Client
    ...
    if st.button("Vérifier"):
        if os.path.exists(storage_file):
            st.write(f"Le fichier {storage_file} existe.")  # Debug
            with open(storage_file, "r") as f:
                stored_data = f.read().splitlines()
            st.write(f"Stored data: {stored_data}")  # Debug
            for record in stored_data:
                ...
                st.write(f"Stored serial: {stored_serial}, Stored date: {stored_date}")  # Debug
                ...
                st.write(f"Decoded serial: {decoded_serial}")  # Debug
