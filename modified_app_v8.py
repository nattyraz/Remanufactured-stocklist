...
def display_data_page():
    ...
    # Format the selected currency column
    
    # Convert the selected currency column to string and remove any commas
    filtered_data[selected_currency] = filtered_data[selected_currency].astype(str).str.replace(',', '')
    
    
    # Format the selected currency column for display without commas
    filtered_data[selected_currency] = 
    # Ensure that Streamlit displays the numbers without commas
    for col in [selected_currency, "Promo Price DKK", "Promo Price GBP"]:
        if col in filtered_data.columns:
            filtered_data[col] = filtered_data[col].apply(lambda x: "{:d}".format(x))
    
    filtered_data[selected_currency] = filtered_data[selected_currency].apply(lambda x: f"{x:,}")
