...
def display_data_page():
    ...
    if combined_data is not None and not combined_data.empty:
        ...
        st.dataframe(filtered_data[columns_to_display].reset_index(drop=True))
        filtered_data[selected_currency] = filtered_data[selected_currency].apply(lambda x : '{:.4f}'.format(x))
