...
def display_pie_chart(data, column_name):
    pie_chart = px.pie(data, names=column_name, title=f"Distribution of {column_name}")
...
def display_data_page():
    ...
    if combined_data is not None and not combined_data.empty:
        ...
        col_format, col_prod_cat, col_keyboard, col_condition = st.columns(4)
        filters = {
            "Format": col_format.multiselect("Format", list(combined_data["Product Group Code"].unique())),
            "Produits catégories": col_prod_cat.multiselect("Produits catégories", list(combined_data["Item Category Code"].unique())),
        ...
        # Display Pie charts for the selected filters
...
def admin_page():
    ...
    # Upload files
    files = st.file_uploader("Importez les fichiers (vous pouvez sélectionner plusieurs fichiers):", type=["xlsx"], accept_multiple_files=True)
