...
def display_data_page():
    ...
    # Add a checkbox column for user to select rows
    filtered_data['select'] = False
    selected_data = st.table(filtered_data.assign(select=st.checkbox('Select all')))

    # Download selected data
    if st.button("Télécharger la sélection"):
        selected_rows = filtered_data[filtered_data['select'] == True]
