...
def load_stock_data(file_path):
    if file_path and os.path.exists(file_path):
        ...
        # Renommer les colonnes de danois à anglais
        df.rename(columns={
            'Nummer': 'Number',
            'Beskrivelse': 'Description',
            'Varekategorikode': 'Item Category Code',
            'Produktgruppekode': 'Product Group Code',
            'Software Sprog': 'Software Language',
            'Keyboard Sprog': 'Keyboard Language',
            'Condition': 'Condition',
            'Garanti': 'Warranty',
            'Lager': 'Stock',
            'Eksport Pris': 'Export Price DKK',
            'Brand': 'Brand'
        }, inplace=True)

        # Conversion des prix de DKK en EUR et GBP
        dkk_to_eur = 7.4634
        dkk_to_gbp = 8.5533
        df['Export Price EUR'] = df['Export Price DKK'] / dkk_to_eur
        df['Export Price GBP'] = df['Export Price DKK'] / dkk_to_gbp
